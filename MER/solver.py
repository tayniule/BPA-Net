...
# ==================== [新增导入] 相关性分析模块 ====================
try:
    from correction_mer import SubjectCorrelationAnalyzer
except ImportError:
    print("错误: 无法找到 correction_mer.py，请确保该文件在当前目录下。")
    raise
...
class Solver(object):
    ...
    #         model_path = '/code/scw/MER/output_mer_group_training/checkpoints/model_mer_group_7_1_2(Valence).pt'
    #
    ...
    #             # 前向传播执行特征提取逻辑
    #             _ = self.model(to_gpu(eeg), to_gpu(eog), to_gpu(emg), to_gpu(l), to_gpu(d), self.groups)
    #
    #             # 获取 solver.py 中定义的两种特征
    ...
    #
    #     # 合并数据
    #     all_shared = np.concatenate(shared_list, axis=0)
    #     all_private = np.concatenate(private_list, axis=0)
    ...
    #         # 增大点的大小 s=30, 使用 tab20 配色以区分更多受试者
    #         scatter = plt.scatter(data_2d[:, 0], data_2d[:, 1], c=all_subjects,
    #                               alpha=0.7, cmap='tab20', s=35)
    #
    #         # 设置标题和字体
    ...
    #     plot_and_save(all_shared, "t-SNE of Shared Features", "./tsne_results/shared_features.png")
    #     plot_and_save(all_private, "t-SNE of Private Features", "./tsne_results/private_features.png")
...
if __name__ == "__main__":
    ...
    base_output_dir = '/code/scw/MER/output_10fold_correction_valence'  # 总输出根目录

    # 降低 Batch Size 以避免内存/显存 OOM
    DataConfig.batch_size = 32

    set_seed(42)

    print("=" * 60)
    print("MER Dataset - 10-Fold Cross Validation")
    print("=" * 60)

    # 2. 获取 10 份 Folds
    folds = get_10_folds_subjects(DataConfig, seed=42)

    # 3. 设置模型超参数 (这里设置一组你认为最优的参数跑 CV)
    base_params = {
        "group_num": 3,  # 这个值会被动态覆盖，这里只是占位
        "diff_weight": 0.05,
        "sim_weight": 0.001,
        "learning_rate": 2e-4
    }
    ...
    for fold_idx in range(10):
        print(f"\n{'#' * 20} Starting FOLD {fold_idx + 1}/10 {'#' * 20}")
        ...
        # --- A. 动态设置当前 Fold 的输出路径 ---
        DataConfig.OUTPUT_ROOT = os.path.join(base_output_dir, f'fold_{fold_idx + 1}')
        DataConfig.LOG_DIR = os.path.join(DataConfig.OUTPUT_ROOT, 'logs')
        DataConfig.CHECKPOINT_DIR = os.path.join(DataConfig.OUTPUT_ROOT, 'checkpoints')
        DataConfig.RESULT_DIR = os.path.join(DataConfig.OUTPUT_ROOT, 'results')
        DataConfig.CURVE_DIR = os.path.join(DataConfig.OUTPUT_ROOT, 'curves')
        setup_directories(DataConfig)

        # --- B. 数据集划分 ---
        train_subs, val_subs, test_subs = get_split_for_fold(folds, fold_idx, seed=42)
        print(f"[Fold {fold_idx + 1} Split]")
        print(f"  Train: {len(train_subs)} subs | Val: {len(val_subs)} subs | Test: {len(test_subs)} subs")

        # --- C. [关键修改] 仅对训练集生成 Groups (防止数据泄露) ---
        # 实例化分析器，传入当前 fold 的输出目录
        ...
        # 运行分析：只传入 train_subs！
        # 这确保了相关性矩阵和聚类结果完全不包含验证/测试集的信息
        print(f"正在分析 Fold {fold_idx + 1} 的训练集被试相关性 (n={len(train_subs)})...")
        correlation_results = analyzer.run_full_analysis(
            subject_ids=train_subs,
            save_groups_for_solver=False  # 结果直接在内存中使用
        )

        # 获取动态生成的分组
        groups = correlation_results['groups']
        current_group_num = len(groups)

        print(f"分析完成: 发现 {current_group_num} 个被试组。")
        for i, g in enumerate(groups):
            print(f"  Group {i + 1} (Size {len(g)}): {g}")

        # 动态更新 group_num 为实际分析出的组数
        current_params = base_params.copy()
        current_params["group_num"] = current_group_num
        # --------------------------------------------

        # --- D. 构建 DataLoader ---
        # 注意 num_workers 根据你服务器性能调整，设为0最稳定
        ...
        train_loader = torch.utils.data.DataLoader(
            train_dataset, batch_size=DataConfig.batch_size, shuffle=True, num_workers=2, pin_memory=True,
            drop_last=True)
        val_loader = torch.utils.data.DataLoader(
            val_dataset, batch_size=DataConfig.batch_size, shuffle=False, num_workers=2, pin_memory=True)
        test_loader = torch.utils.data.DataLoader(
            test_dataset, batch_size=DataConfig.batch_size, shuffle=False, num_workers=2, pin_memory=True)

        # --- E. 配置 Solver 并训练 ---
        train_config = DataConfig()
        train_config.name = f"cv_fold_{fold_idx + 1}"
        train_config.n_epoch = 60
        train_config.patience = 8
        train_config.cls_weight = 1.0
        train_config.weight_decay = 1e-4
        train_config.optimizer = torch.optim.RAdam
        ...
        try:
            ...
            print(f"  [Fold {fold_idx + 1} Success] Test Acc: {test_acc:.4f}, Test F1: {test_f1:.4f}")
    ...
    # 5. 输出 10-Fold 综合测试报告
    print("\n" + "=" * 80)
    print("10-FOLD CROSS VALIDATION SUMMARY REPORT")
    print("=" * 80)

    if not all_fold_results:
        print("No folds completed successfully.")
    else:
        header = f"{'Fold':<6} | {'Groups':<6} | {'Best Epoch':<10} | {'Val Acc':<8} | {'Test Acc':<8} | {'Test F1':<8} | {'Test Kappa':<10}"
        print(header)
        print("-" * len(header))
        ...
        for res in all_fold_results:
            ...
            kappas.append(res['test_kappa'])
            print(f"{res['fold']:<6} | {res['groups']:<6} | {res['best_epoch']:<10} | {res['best_val_acc']:<8.4f} | "
                  f"{res['test_acc']:<8.4f} | {res['test_f1']:<8.4f} | {res['test_kappa']:<10.4f}")

        print("-" * len(header))
        print(
            f"MEAN   |        |            |          | {np.mean(accs):<8.4f} | {np.mean(f1s):<8.4f} | {np.mean(kappas):<10.4f}")
        print(
            f"STD    |        |            |          | {np.std(accs):<8.4f} | {np.std(f1s):<8.4f} | {np.std(kappas):<10.4f}")

        # 保存总体总结 CSV
        summary_path = os.path.join(base_output_dir, '10_fold_summary.csv')
        try:
    ...
    print(f"\nTotal CV time: {total_time:.2f} hours")